...
export function IntegrationHealthBanner({ connectors }: IntegrationHealthBannerProps) {
  ...
  const cards = [
    { label: "Ecosystem Sync Health", value: `${healthScore}%`, icon: ShieldCheck, color: "text-emerald-500" },
    { label: "Active Enterprise Connectors", value: `${activeCount} / ${connectors.length}`, icon: Activity, color: "text-accent" },
    { label: "Synced Records Stream", value: totalRecords.toLocaleString(), icon: Database, color: "text-foreground" },
    { label: "Sync Engine Status", value: "Operational", icon: RefreshCw, color: "text-emerald-500" },
  ];
