  ...
  return (
    <div ref={dropdownRef} className={cn("relative inline-block", className)} {...props}>
      <button
        ...
        className="relative flex items-center justify-center w-8 h-8 rounded-md text-muted-foreground hover:text-foreground bg-card/80 hover:bg-secondary border border-border transition-colors focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring"
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-card text-card-foreground rounded-lg border border-border shadow-lg z-50 overflow-hidden animate-in fade-in-50 zoom-in-95 duration-100">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-secondary/30">
            <div className="flex items-center gap-2">
              ...
              {unreadCount > 0 && (
                <span className="text-[10px] bg-accent/15 text-accent font-semibold px-1.5 py-0.5 rounded-full border border-accent/30">
                  {unreadCount} new
                </span>
              )}
            </div>
            ...
            {notifications.map((n) => (
              ...
              >
                <div className="shrink-0 mt-0.5">
                  {n.type === "ai" && <Sparkles className="w-4 h-4 text-accent" />}
